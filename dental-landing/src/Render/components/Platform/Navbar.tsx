...
import { pathsIncludes } from '../../../types';
...
const Navbar = () => {
  ...
  useEffect(() => {
    const routeToTitleMap: [pathsIncludes, string][] = [
    ...
    const matchedRoute = routeToTitleMap.find(([route]) =>
      pathname.includes(route)
    );

    if (typeof matchedRoute !== 'undefined') {
      setTitle(matchedRoute[1]);
    } else {
      setTitle('');
    }
  }, [location]);
            ...
            <div className="mt-[70px]">
              ...
              <div className="mt-7">
                <button
                  ...
                  onClick={logout}>
                  Cerrar sesión
                  <span className="text-[20px] text-black ml-5">
                    <IoLogOutOutline />
                  </span>
                </button>
              </div>
      ...
      <header className="h-[127px] bg-[#DAE3DE] fixed top-0 w-full hidden lg:flex z-10">
        <div className="flex justify-between items-center w-10/12 mx-auto pl-[120px]">
          <div className="flex justify-center items-center">
            ...
            <span className="-ml-2 text-[1.7rem] font-semibold">{title}</span>
          </div>

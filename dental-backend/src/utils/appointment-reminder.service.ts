...
export class AppointmentReminderService {
    ...
    constructor(
        ...
        cron.schedule('* * * * *', () => {
            this.logger.debug('Ejecutando tarea de recordatorio de citas cada minuto');
            ...
            if(appointments.length > 0) {
                for (const appointment of appointments) {
                    ...
                    if (
                        ...
                        this.logger.debug(`Cita no cumple la condición para recordatorio: ${appointment.date}`);
                    }
                }
            }else {
                return
